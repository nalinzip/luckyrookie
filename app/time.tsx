...
const DAY_TIME_RANGES: Record<number, DayTimeRanges> = {
  1: { lucky: ["06:01 - 07:00", "13:31 - 19:30"], love: ["20:01 - 24:00"], unlucky: ["07:01 - 13:00", "00:01 - 03:00"] },
  2: { lucky: ["15:01 - 16:00", "19:01 - 21:00"], love: ["10:01 - 13:00", "22:01 - 01:01"], unlucky: ["03:01 - 06:30"] },
  3: { lucky: ["06:01 - 09:30", "14:30 - 19:30"], love: ["21:01 - 01:00", "22:01 - 01:01"], unlucky: ["09:01 - 14:00"] },
  4: { lucky: ["06:01 - 07:00", "09:01 - 13:00"], love: ["13:31 - 19:30"], unlucky: ["07:01 - 09:00", "01:01 - 03:00"] },
  5: { lucky: ["06:01 - 12:00", "15:31 - 16:00"], love: ["19:01 - 23:00"], unlucky: ["12:01 - 15:00", "23:01 - 03:00"] },
  6: { lucky: ["15:01 - 23:00"], love: ["07:01 - 14:00"], unlucky: ["14:01 - 15:00", "01:01 - 05:00"] },
  0: { lucky: ["07:31 - 12:30", "20:01 - 24:00"], love: ["12:31 - 13:30", "01:01 - 03:00"], unlucky: ["06:31 - 07:30", "16:01 - 17:00"] },
};
...
function getCurrentTimeStatus(
  dayTimeRanges: DayTimeRanges | null,
  currentTime: Date
): { status: "lucky" | "love" | "unlucky" | "neutral"; message: string } | null {
  if (!dayTimeRanges) return null;

  for (const range of dayTimeRanges.lucky) {
    if (isTimeInRange(currentTime, range)) return { status: "lucky", message: "Lucky Time!" };
  }
  for (const range of dayTimeRanges.love) {
    if (isTimeInRange(currentTime, range)) return { status: "love", message: "Charming Time!" };
  }
  for (const range of dayTimeRanges.unlucky) {
    if (isTimeInRange(currentTime, range)) return { status: "unlucky", message: "Avoid this time" };
  }
...
export default function App() {
  ...
  return (
    <LinearGradient colors={["#EEF7F1", "#F3F0FF"]} style={styles.root}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.timeRangeSection}>
              ...
              {dayTimeRanges.lucky.map((range, idx) => (
                <Text key={idx} style={styles.timeRangeText}>
                  {range}
                </Text>
              ))}
            </View>
            ...
            <View style={styles.timeRangeSection}>
              ...
              {dayTimeRanges.unlucky.map((range, idx) => (
                <Text key={idx} style={[styles.timeRangeText, { color: "#F44336" }]}>
                  {range}
                </Text>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 18, fontWeight: "800", marginBottom: 16, color: "#222222" },

  inputGroup: { marginBottom: 16 },
  label: { marginBottom: 8, color: "#444", fontWeight: "600", fontSize: 15 },
  input: {
    borderWidth: 1,
    borderColor: "#E0E0E0",
    borderRadius: 12,
  ...
  birthDayName: { fontSize: 24, fontWeight: "800", color: "#9810FA" },
  ...
  currentTimeLucky: { backgroundColor: "#E8F5E9", borderColor: "#4CAF50" },
  currentTimeLove: { backgroundColor: "#FCE4EC", borderColor: "#FF69B4" },
  currentTimeUnlucky: { backgroundColor: "#FFEBEE", borderColor: "#F44336" },
  currentTimeNeutral: { backgroundColor: "#F5F5F5", borderColor: "#E0E0E0" },
  currentTimeLabel: { fontSize: 11, fontWeight: "700", color: "#666", letterSpacing: 1.5, marginBottom: 8 },
  currentTimeTime: { fontSize: 36, fontWeight: "900", color: "#222", marginBottom: 8, letterSpacing: -1 },
  currentTimeStatus: { fontSize: 16, fontWeight: "700", color: "#222" },

  timeRangeSection: { marginBottom: 20 },
  timeRangeHeader: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 10 },
  timeRangeDot: { width: 12, height: 12, borderRadius: 6 },
  timeRangeTitle: { fontSize: 16, fontWeight: "700", color: "#222" },
  timeRangeText: { fontSize: 15, fontWeight: "600", color: "#444", marginLeft: 22, marginBottom: 6 },

  personalDayHeader: { flexDirection: "row", alignItems: "center", gap: 16 },
  personalDayBadge: {
    width: 64,
    height: 64,
    borderRadius: 32,
  ...
  personalDayNumber: { fontSize: 32, fontWeight: "900", color: "#9810FA" },
  personalDayTextContainer: { flex: 1 },
  personalDayLabel: { fontSize: 16, fontWeight: "700", color: "#222", marginBottom: 4 },
  personalDaySubtext: { fontSize: 13, color: "#666", fontWeight: "500" },
  ...
  countdownTime: { fontSize: 48, fontWeight: "900", color: "#FFFFFF", marginBottom: 8, letterSpacing: -1 },
  countdownDetail: { fontSize: 16, fontWeight: "500", color: "#FFFFFF", opacity: 0.95 },
  ...
  timeLeft: { flex: 1 },
  timeLabel: { fontWeight: "600", color: "#444", fontSize: 15 },
  timeRight: { fontWeight: "700", color: "#9810FA", fontSize: 16 },

  placeText: { color: "#666", fontSize: 14, marginBottom: 20, fontWeight: "500" },

  goldenHourRow: { flexDirection: "row", gap: 16 },
  goldenHourItem: {
    flex: 1,
    alignItems: "center",
    backgroundColor: "#FAFAFA",
  ...
  goldenHourDivider: { width: 1, backgroundColor: "#E0E0E0" },
  goldenHourLabel: {
    fontSize: 13,
    fontWeight: "600",
    color: "#666",
  ...
  goldenHourTime: { fontSize: 14, fontWeight: "700", color: "#222", textAlign: "center" },
});
